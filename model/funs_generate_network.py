...
def get_network(plann_steps_max=10):
    ...
    # Upload dike info
    df = pd.read_excel("./model/data/dikeIjssel.xlsx", dtype=object)
    ...
    frag_curves = pd.read_excel(
        "./model/data/fragcurves/frag_curves.xlsx", header=None, index_col=0
    ).transpose()
    calibration_factors = pd.read_excel(
        "./model/data/fragcurves/calfactors_pf1250.xlsx", index_col=0
    )
    ...
    projects = pd.read_excel(
        "./model/data/rfr_strategies.xlsx", index_col=0, names=["project name", 0, 1, 2, 3, 4]
    )
    ...
    G.add_node("EWS", **pd.read_excel("./model/data/EWS.xlsx").to_dict())
    G.nodes["EWS"]["type"] = "measure"

    # Upload muskingum params:
    Muskingum_params = pd.read_excel("./model/data/Muskingum/params.xlsx", index_col=0)
    ...
    for dike in dike_list:
        ...
        filename = f"./model/data/rating_curves/{dike}_ratingcurve_new.txt"  # Load file
        ...
        name = f"./model/data/losses_tables/{dike}_lossestable.xlsx"
    ...
    G.nodes["A.0"]["Qevents_shape"] = pd.read_excel(
        "./model/data/hydrology/wave_shapes.xls", index_col=0
    )
